...
def get_password_info(raw_database_entry: str) -> tuple:
    exp = r"(?P<low>\d+)-(?P<high>\d+)\s(?P<char>\w):\s(?P<password>\w+)"
    match = re.match(exp, raw_database_entry)
    return (
        int(match.group("low")),
...
def solve_part1(db_entries: list) -> int:
    def is_password_valid(password_info: tuple) -> bool:
        lower_limit, upper_limit, char, password = password_info
        char_count = password.count(char)
        return char_count in range(lower_limit, upper_limit + 1)

    passwords = map(get_password_info, db_entries)
    
    count = 0
    for item in passwords:
        if is_password_valid(item):
            count += 1
    return count
...
assert solve_part1(test_database_entries) == 2
    

##### Part 2 #####

def solve_part2(db_entries: list) -> int:
    def is_password_valid(password_info: tuple) -> bool:
        pos1, pos2, char, password = password_info
        is_char_at_pos1 = password[pos1 - 1] == char
        is_char_at_pos2 = password[pos2 - 1] == char
        return is_char_at_pos1 ^ is_char_at_pos2
...
assert solve_part2(test_database_entries) == 1

##### Main #####

def main():
    with open('input.txt') as f:
        password_db_entries = [line for line in f]
        part1 = solve_part1(password_db_entries)
        part2 = solve_part2(password_db_entries)
