...
def find_col_for_boarding_pass(boarding_pass: str) -> int:
    col_rules = boarding_pass[7:]
    cols = list(range(8))
...
def find_row_for_boarding_pass(boarding_pass: str) -> int:
    row_rules = boarding_pass[:7]
    rows = list(range(128))
...
def solve_part1(boarding_passes: list) -> int:
    seat_ids = map(generate_seat_id, boarding_passes)
    return max(seat_ids)


def solve_part2(boarding_passes: list) -> int:
    seat_ids = list(map(generate_seat_id, boarding_passes))
    seat_ids.sort()
...
def main():
    with open("input.txt") as f:
        boarding_passes = [line for line in f]
        part1 = solve_part1(boarding_passes=boarding_passes)
        part2 = solve_part2(boarding_passes=boarding_passes)
