...
def solve_part1(numbers: list, preamble_length: int) -> int:
    PREAMBLE_LENGTH = preamble_length

    for index, number in enumerate(
        numbers[PREAMBLE_LENGTH:], start=PREAMBLE_LENGTH + 1
    ):
        the_preamble = numbers[index - PREAMBLE_LENGTH - 1 : index - 1]

        found = False
        for num in the_preamble:
            if number - num in the_preamble:
                found = True
                break
...
def solve_part2(numbers: list, preamble_length: int) -> int:
    invalid_number = solve_part1(numbers=numbers, preamble_length=preamble_length)

    for set_length in range(2, int(len(numbers) / 2)):
        i = 0
        while i < len(numbers):
            numbers_set = numbers[i : i + set_length]
            if sum(numbers_set) == invalid_number:
...
def main():
    ...
    part1 = solve_part1(numbers=numbers, preamble_length=25)
    part2 = solve_part2(numbers=numbers, preamble_length=25)
