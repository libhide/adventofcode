...
def first(items: list, condition: Callable[[Any], bool]) -> Any:
    for item in items:
        if condition(item):
            return item
...
def solve_part1(jolts: list) -> int:
    ...
    for i in range(len(jolts)):
        ...
        next_joltage = first(possible_next_joltages, lambda x: x in jolts)
        d[next_joltage - prev_joltage] += 1
        prev_joltage = next_joltage

    return d[1] * d[3]


def solve_part2(jolts: list) -> int:
    path_ways = {}

    def traverse(current: int) -> int:
        if current == len(jolts) - 1:
            return 1

        if current in path_ways:
            return path_ways[current]

        path_count = 0
        for i in range(current + 1, len(jolts)):
            if jolts[i] - jolts[current] <= 3:
                path_count += traverse(i)

        path_ways[current] = path_count
        return path_count
...
def main():
    ...
    part1 = solve_part1(jolts=jolts)
    part2 = solve_part2(jolts=jolts)
