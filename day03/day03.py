...
def calc_slope(grid: list, right_inc: int, down_inc: int) -> int:
...
def solve_part1(grid: list) -> int:
    return calc_slope(grid=grid, right_inc=3, down_inc=1)


def solve_part2(grid: list) -> int:
    result = (
        calc_slope(grid=grid, right_inc=1, down_inc=1)
        * calc_slope(grid=grid, right_inc=3, down_inc=1)
        * calc_slope(grid=grid, right_inc=5, down_inc=1)
        * calc_slope(grid=grid, right_inc=7, down_inc=1)
        * calc_slope(grid=grid, right_inc=1, down_inc=2)
    )
    return result
...
def main():
    with open("input.txt") as f:
        grid = [list(line.strip() * 100) for line in f]
        part1 = solve_part1(grid=grid)
        part2 = solve_part2(grid=grid)
